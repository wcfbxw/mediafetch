...
规范化的来源域名白名单，系统只会对精确域名或其子域名选中 Hook。内置的抖音 Hook 仅添加
服务端固定的官方页面 Referer，并校验 yt-dlp 的平台结果。

Hook 接口刻意不提供修改或返回媒体 URL、格式选择器、yt-dlp 参数、FFmpeg 参数和输出路径的
能力。所有格式仍由 yt-dlp 官方提取器正常返回，原始媒体 URL 不会被 Hook 改写；SSRF 重定向
校验、Redis 缓存与队列、Job ID 以及签名文件交付流程保持不变。新增平台 Hook 时必须通过代码
审查和对应的域名隔离测试，运行时不能从用户输入动态加载插件。
